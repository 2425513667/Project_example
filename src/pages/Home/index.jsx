...
const headCells = [
  {
    ...
    numeric: false,
    disablePadding: true,
    label: <FormattedMessage id="postId" />,
  },
  {
    id: "postCode",
    numeric: true,
  ...
  },
  {
    ...
    label: (
      <>
        <FormattedMessage id="CommodityCategory" />
...
export default function Home() {
  ...
  const [category_Id, setPoset_id] = useState(); //删除的id
  const [post_name, setPost_name] = useState(); //删除的post_name

  const handleClickOpen = (event, categoryId, postName) => {
    setPoset_id(categoryId);
  ...
  const handleDelete = () => {
    ...
    rows.splice(
      rows.findIndex((post) => post.categoryId === category_Id),
      1
    );
  };
  ...
  //修改
  const handleClickOpenupdate = (categoryId) => {
    setupdateindex(rows.findIndex((post) => post.categoryId === categoryId));
    setDuihua(true);
  };
  ...
  const postADD = () => {
    let d = huoqutianjiazi;
    d["categoryId"] = rows[rows.length - 1].categoryId + 1;
    sethuoqutianjiazi(d);
    let pyadd = rows;
  ...
  return (
    <div className={classes.root}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Button
                          color="primary"
                          onClick={() => handleClickOpenupdate(row.categoryId)}
                        >
                          <BorderColor />
                          修改
                        </Button>
                        ...
                        <Button
                          color="primary"
                          onClick={(event) =>
                            handleClickOpen(event, row.categoryId, row.postName)
                          }
                        >
                          <Delete />
                          删除
      ...
      <Dialog open={duihua} onClose={duihuaClose}>
        <DialogTitle>
          <h1>{rows[updateindex]?.categoryId ? "修改" : "新增"}</h1>
        </DialogTitle>
        <DialogContent>
          <ListItem>
          ...
          <ListItem style={{ marginTop: "15px", marginBottom: "15px" }}>
            <Button
              ...
              onClick={rows[updateindex]?.categoryId ? postUpdate : postADD}
              variant="contained"
              color="primary"
            >
              确定
      ...
      <StyledDialog
        ...
        content={`是否删除编号：${category_Id} , 类别: ${post_name}这条记录吗?`}
      />
    </div>
  );
}
